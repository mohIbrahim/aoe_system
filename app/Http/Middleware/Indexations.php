...
class Indexations
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if($request->route()->getName() == 'indexations.create'     && in_array('create_indexations', $permissions)){

            $response = $next($request);
        }else

        if($request->route()->getName() == 'indexations.store'      && in_array('create_indexations', $permissions)){

            $response = $next($request);
        }else

        if($request->route()->getName() == 'indexations.edit'       && in_array('update_indexations', $permissions)){

            $response = $next($request);
        }else

        if($request->route()->getName() == 'indexations.update'     && in_array('update_indexations', $permissions)){

            $response = $next($request);
        }else

        if($request->route()->getName() == 'indexations.destroy'    && in_array('delete_indexations', $permissions)){
