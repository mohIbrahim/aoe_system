...
$(document).ready(function () {
	if ($('.standard-datatable').DataTable) {
		...
		$(theTable.table().container()).on('keyup', 'tfoot input', function () {
			theTable.column($(this).data('index')).search(this.value).draw();
		});
...
$(document).ready(function () {
	...
	$('input').keypress(function (event) {
		//prevent enter key to submit the form and coz we was have a bug when pressing enter key inside input field another telecom input added
		if (event.keyCode == 13) {
			event.preventDefault();
		}
	});
...
$(document).ready(function () {
	...
	$(upload_files_pdf_add_button).click(function (e) {
		//on add input button click
		e.preventDefault();
		if (x < upload_files_pdf_max_fields) {
			//max input box allowed
			x++; //text box increment
	...
	$(upload_files_pdf_wrapper).on("click", ".upload_files_pdf_remove_field", function (e) {
		//user click on remove text
		e.preventDefault();$(this).parent('div').remove();x--;
	});
});
...
$(document).ready(function () {
	...
	$(upload_files_img_add_button).click(function (e) {
		//on add input button click
		e.preventDefault();
		if (x < upload_files_img_max_fields) {
			//max input box allowed
			x++; //text box increment
	...
	$(upload_files_img_wrapper).on("click", ".upload_files_img_remove_field", function (e) {
		//user click on remove text
		e.preventDefault();$(this).parent('div').remove();x--;
	});
});
			...
			$.ajax({
				...
				success: function success(results) {
					...
					$.each(results, function (index, contract) {
						standardTable.row.add([index + 1, "<a href='/contracts/" + contract.id + "'>" + contract.code + "</a>", contract.type, contract.start, contract.end, contract.status, contract.payment_system, contract.printing_machines[0] ? contract.printing_machines[0].customer ? contract.printing_machines[0].customer.name : '' : '', contract.price]);
					});
					standardTable.draw();
			...
			$.ajax({
				...
				success: function success(results) {
					...
					$.each(results, function (index, employee) {
						standardTable.row.add([index + 1, "<a href='/employees/" + employee.id + "' target='_blank'>" + (employee.user ? employee.user.name : '') + "</a>", employee.code, employee.job_title, employee.department ? employee.department.name : 'لا يوجد', employee.the_department_that_he_manage_it ? employee.the_department_that_he_manage_it.name : 'لا يوجد', employee.date_of_hiring]);
					});
					standardTable.draw();
					$("#my-table-body").fadeIn();
				}
			...
			$.ajax({
				...
				success: function success(results) {
					...
					$.each(results, function (index, follow_up_card_special_report) {
						standardTable.row.add([index + 1, follow_up_card_special_report.id, "<a href='/follow_up_card_special_reports/" + follow_up_card_special_report.id + "'>" + follow_up_card_special_report.the_date + "</a>", follow_up_card_special_report.readings_of_printing_machine, follow_up_card_special_report.indexation_number, follow_up_card_special_report.invoice_number, follow_up_card_special_report.the_payment]);
					});
					standardTable.draw();
					$("#my-table-body").fadeIn();
				}
			...
			$.ajax({
				...
				success: function success(results) {
					...
					$.each(results, function (index, follow_up_card) {
						standardTable.row.add([index + 1, "<a href='/follow_up_cards/" + follow_up_card.id + "'>" + follow_up_card.code + "</a>", follow_up_card.old_code, follow_up_card.printing_machine !== null ? follow_up_card.printing_machine.customer !== null ? follow_up_card.printing_machine.customer.name : '' : '', follow_up_card.contract ? "<a href='/contracts/" + follow_up_card.contract.id + "'>" + follow_up_card.contract.code + "</a>" : '', follow_up_card.printing_machine !== null ? follow_up_card.printing_machine.serial_number : '', follow_up_card.printing_machine ? follow_up_card.printing_machine.assigned_employees[0] ? follow_up_card.printing_machine.assigned_employees[0].user ? follow_up_card.printing_machine.assigned_employees[0].user.name : '' : '' : '', formatDate(follow_up_card.created_at) || '', formatDate(follow_up_card.updated_at) || '']);
					});
					standardTable.draw();
					$("#my-table-body").fadeIn();
				}
			...
			$.ajax({
				...
				success: function success(results) {
					...
					$.each(results, function (index, indexation) {
						standardTable.row.add([index + 1, "<a href='/indexations/" + indexation.id + "'>" + indexation.code + "</a>", indexation.printing_machine ? indexation.printing_machine.customer ? indexation.printing_machine.customer ? "<a href='/customers/" + indexation.printing_machine.customer.id + "'>" + indexation.printing_machine.customer.name + "</a>" : '' : '' : indexation.visit ? indexation.visit.printing_machine ? indexation.visit.printing_machine.customer ? "<a href='/customers/" + indexation.visit.printing_machine.customer.id + "'>" + indexation.visit.printing_machine.customer.name + "</a>" : '' : '' : '', indexation.printing_machine ? "<a href='/printing_machines/" + indexation.printing_machine.id + "' targe='_blank'>" + indexation.printing_machine.serial_number + "</a>" : indexation.visit ? indexation.visit.printing_machine ? "<a href='/printing_machines/" + indexation.visit.printing_machine.id + "' targe='_blank'>" + indexation.visit.printing_machine.serial_number + "</a>" : '' : '', indexation.the_date, indexation.employee_who_performed_indexation, indexation.customer_approval, indexation.technical_manager_approval, indexation.warehouse_approval, indexation.type, indexation.visit ? indexation.visit.id : '', indexation.visit ? indexation.visit.readings_of_printing_machine : '', indexation.totalPrice]);
					});
					standardTable.draw();
					$("#my-table-body").fadeIn();
				}
				...
				if (results) {
					...
					$(".invoice-form-part-add-button").on("click", function () {
						...
						$('.invoice-form-delete-part-button').on('click', function () {
							$(this).parent().parent().parent().fadeOut('500', 'linear', function () {
								$(this).remove();
							});
						});
					});
...
window.invoiceFormViewChooseACustomer = function () {
	...
	function renderSearchResults(results) {
		if (results) {
			...
			$.each(results, function (index, item) {
				rows += addRow([index + 1, item.name, item.code, '<button type="button" class="btn btn-danger btn-xs select-cutomer-button" data-selected-customer-id="' + item.id + '" data-selected-customer-name="' + item.name + '">إختيار</button>']);
			});
	...
	function selectCustomer() {
		id = $(this).data("selected-customer-id"), name = $(this).data("selected-customer-name");
		customer = {
			name: name,
			id: id
		};
			...
			$.ajax({
				...
				success: function success(results) {
					...
					$.each(results, function (index, part) {
						standardTable.row.add([index + 1, part.part ? part.part.name : '', "<a href='/part_serial_numbers/" + part.id + "'>" + part.serial_number + "</a>", part.code, part.availability, part.status, part.date_of_entry, part.date_of_departure]);
					});
					standardTable.draw();
					$("#my-table-body").fadeIn();
				}
			...
			$.ajax({
				...
				success: function success(results) {
					...
					$.each(results['parts'], function (index, part) {
						standardTable.row.add([index + 1, "<a href='/parts/" + part.id + "'>" + part.name + "</a>", part.code, results['view_part_number'] ? part.part_number : 'محجوب', part.type, part.is_serialized ? part.qty : part.no_serial_qty, part.compatible_printing_machines]);
					});
					standardTable.draw();
					$("#my-table-body").fadeIn();
				}
			...
			$.ajax({
				...
				success: function success(results) {
					...
					$.each(results, function (index, reference) {
						standardTable.row.add([index + 1, "<a href='/references/" + reference.id + "'>" + reference.code + "</a>", reference.employee_who_receive_the_rereference ? reference.employee_who_receive_the_rereference.user ? reference.employee_who_receive_the_rereference.user.name : '' : '', reference.type, reference.status, reference.assigned_employee ? reference.assigned_employee.user ? reference.assigned_employee.user.name : '' : '', reference.received_date, "<a href='/printing_machines/" + (reference.printing_machine ? reference.printing_machine.id : '') + "'>" + (reference.printing_machine ? reference.printing_machine.code : '') + "</a>", reference.printing_machine ? reference.printing_machine.serial_number ? reference.printing_machine.serial_number : '' : '', reference.printing_machine ? reference.printing_machine.folder_number ? reference.printing_machine.folder_number : '' : '', reference.printing_machine ? reference.printing_machine.customer ? reference.printing_machine.customer.name ? reference.printing_machine.customer.name : '' : '' : '', reference.visits.length > 0 ? '<a href="/visits/' + reference.visits[reference.visits.length - 1].id + '">' + reference.visits[reference.visits.length - 1].id + '</a>' : '', reference.reviewed_by_the_chief_engineer ? reference.reviewed_by_the_chief_engineer : '']);
					});
					standardTable.draw();
					$("#my-table-body").fadeIn();
				}
			...
			$.ajax({
				...
				success: function success(results) {
					...
					$.each(results, function (key, visit) {
						standardTable.row.add([key + 1, "<a href='/visits/" + visit.id + "' target='_blank'>" + visit.id + "</a>", visit.visit_date, visit.type, visit.printing_machine !== null ? "<a href='/printing_machines/" + visit.printing_machine.id + "' target='_blank'>" + visit.printing_machine.code + "</a>" : '', visit.readings_of_printing_machine, visit.the_employee_who_made_the_visit.user !== null ? visit.the_employee_who_made_the_visit.user.name : '']);
					});
