...
<div class="form-group">
    ...
    <select class="form-control" name="type">
        <?php $customerType = isset($cutomer->type)? $customer->type:'' ;?>
        <option value="">  أختر نوع العميل.  </option>
        <option value="indeviduals" {{($customerType == 'indeviduals')? 'selected' : ((old('type')=='indeviduals')?'selected':'')}}> أفراد </option>
        <option value="companies" {{($customerType == 'companies')? 'selected' : ((old('type')=='companies')?'selected':'')}}> شركات </option>
        <option value="government_authorities" {{($customerType == 'government_authorities')? 'selected' : ((old('type')=='government_authorities')?'selected':'')}}> هيئات حكومية </option>
        <option value="schooles" {{($customerType == 'schooles')? 'selected' : ((old('type')=='schooles')?'selected':'')}}> مدارس </option>
        <option value="hospitals" {{($customerType == 'hospitals')? 'selected' : ((old('type')=='hospitals')?'selected':'')}}> مستشفيات </option>
        <option value="banks" {{($customerType == 'banks')? 'selected' : ((old('type')=='banks')?'selected':'')}}> بنوك </option>
    </select>
</div>
...
<div class="input_fields_wrap_1">
    <div class="form-group">
        ...
        <input type="text" class="form-control" id="telecom" name="telecom[]"  placeholder=" إدخل التليفون. " value="{{$customer->telecom or old('telecom')}}">
